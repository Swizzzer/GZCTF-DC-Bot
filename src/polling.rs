...
impl PollingService {
    ...
    async fn check_match(&self, ctx: &Context, match_config: &MatchConfig) -> Result<()> {
        ...
        for notice_type in &notice_types {
            self.handle_notices(ctx, match_config, notice_type, &notices, &mut tracker)
                .await;
        }
        Ok(())
    }
    ...
    async fn broadcast(
        ...
        for notice in notices {
            self.broadcast_single(ctx, match_config, notice_type, notice)
                .await
    ...
    async fn poll_matches(&self, ctx: &Context, matches: &[MatchConfig]) {
        for match_config in matches {
            self.check_match(ctx, match_config)
                .await
                .unwrap_or_else(|e| {
                    log::error(format!(
